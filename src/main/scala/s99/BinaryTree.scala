...
//Case class inheritance breaks equivalence relation (symmetric and transitive).
// Any case class successor which constricts a definition area must redefine the equality because pattern matching must work exactly as equality.
//https://blog.knoldus.com/case-class-inheritance/
case class PositionedNode[+T]( val value: T,  val left: Tree[T],  val right: Tree[T], x: Int, y: Int) extends Tree[T] {
  ...
  override def layoutBinaryTreeInternal(x: Int, depth: Int): (Tree[T], Int) = ???

  override def treeDepth: Int = ???

  override def leftmostNodeDepth: Int = ???

  override def layoutBinaryTree2Internal(x: Int, depth: Int, exp: Int): Tree[T] = ???

  override def bounds: List[(Int, Int)] = ???

  override def layoutBinaryTree3Internal(x: Int, depth: Int): Tree[T] = ???
